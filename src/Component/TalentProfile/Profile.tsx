...
// import { getProfile, downloadResume, deleteResume } from "../../Services/ProfileService"
...
import { Avatar, Button, Divider } from "@mantine/core";
import { IconBriefcase, IconMapPin, IconFileText } from "@tabler/icons-react";
import ExpCard from "./ExpCard";
import CertiCard from "./CertiCard";
import { useParams } from "react-router-dom";
import { useEffect, useState } from "react";
    ...
    return (
        <div className="w-2/3 lg-mx:w-full">
            ...
            <div className="relative">
                ...
                <div className="flex items-center justify-center absolute -bottom-1/4 md-mx:-bottom-10 sm-mx:-bottom-16 left-6">
                    <Avatar
                        className="!h-48 !w-48 md-mx:!w-40 md-mx:!h-40 
                                   sm-mx:!h-36 sm-mx:!w-36 xs-mx:!h-32 xs-mx:!w-32 
                                   border-mine-shaft-950 border-8 rounded-full"
                        src={
                            profile?.picture
            ...
            <div className="px-3">
                ...
                <div className="flex flex-wrap gap-2">
                    {profile?.skills?.map((skill: any, idx: number) => (
                        <div
                            key={idx}
                            className="bg-bright-sun-300 text-sm font-medium bg-opacity-15 
                                       rounded-3xl text-bright-sun-400 px-3 py-1"
                        >
                            {skill}
                        </div>
                    ))}
            ...
            <div className="px-3">
                <div className="text-2xl font-semibold mb-5">Certification</div>
                <div className="flex flex-col gap-8">
                    {profile?.certifications?.map((certi: any, idx: number) => (
                        <CertiCard key={idx} {...certi} />
                    ))}
                </div>
            </div>
        </div>
