...
const SignUpPage = () => {
    ...
    return (
        <div className="h-screen bg-mine-shaft-950 font-['Nunito'] relative overflow-hidden">
            <LoadingOverlay
                visible={loading}
                zIndex={100}
                overlayProps={{ blur: 2 }}
            ...
            {/* Container for Login and SignUp - Slides on mobile */}
            <div className={`
                flex w-[200%] bs:w-full h-full transition-transform duration-700 ease-in-out
                ${isSignUp ? '-translate-x-1/2 bs:translate-x-0' : 'translate-x-0'}
            `}>
                {/* 1. Login Panel */}
            ...
            {/* 3. Branding Panel (Desktop Only) */}
            <div
                className={`
                    hidden bs:flex
                    w-1/2 h-full bg-mine-shaft-900 
                    transition-all duration-1000 ease-in-out 
                    gap-5 flex-col items-center justify-center
                    absolute top-0 z-20
